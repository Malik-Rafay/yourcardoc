...
import { CURRENCIES, LANGUAGES, REGIONS, useLocale, type Currency, type Language, type Region } from "@/lib/i18n";
  ...
  return (
    <div className="w-full border-b border-border/60 bg-card/50 print:hidden">
      <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-end gap-2 px-4 py-1.5 text-xs sm:px-6">
        <LocaleSelect icon={<MapPin className="h-3 w-3" />} label={t("locale.region")} value={region} onChange={(v) => setRegion(v as Region)}
          options={REGIONS.map((r) => ({ value: r.code, label: r.label }))} />
        <LocaleSelect icon={<Globe className="h-3 w-3" />} label={t("locale.language")} value={language} onChange={(v) => setLanguage(v as Language)}
          options={LANGUAGES.map((l) => ({ value: l.code, label: l.label }))} />
        <LocaleSelect icon={<Coins className="h-3 w-3" />} label={t("locale.currency")} value={currency} onChange={(v) => setCurrency(v as Currency)}
...
function LocaleSelect({ icon, label, value, onChange, options }: {
